...
# Import bidirectional interactor splitting
try:
    from utils.aggregation import split_bidirectional_interactor
    BIDIRECTIONAL_SPLIT_AVAILABLE = True
except ImportError:
    BIDIRECTIONAL_SPLIT_AVAILABLE = False
    split_bidirectional_interactor = None
...
# --- Cached Gemini client (reuse across pipeline steps) ---
...
def _get_gemini_client(api_key: str):
    ...
    with _gemini_client_lock:
        if api_key not in _gemini_client_cache:
            _gemini_client_cache[api_key] = google_genai.Client(api_key=api_key)
...
def call_gemini_model(step: StepConfig, prompt: str, cancel_event=None) -> tuple[str, Dict[str, int]]:
    ...
    # Reuse a single client per API key to avoid per-call connection overhead
...
def run_full_job(
    ...
    try:
        ...
        # --- STAGE 4.5: Split bidirectional interactors into separate entries ---
        if BIDIRECTIONAL_SPLIT_AVAILABLE and split_bidirectional_interactor is not None:
            main_protein_name = validated_payload.get("ctx_json", {}).get("main", user_query)
            for json_key in ["ctx_json", "snapshot_json"]:
                if json_key in validated_payload and "interactors" in validated_payload[json_key]:
                    original_interactors = validated_payload[json_key]["interactors"]
                    split_interactors = []
                    for interactor in original_interactors:
                        split_result = split_bidirectional_interactor(interactor, main_protein_name)
                        split_interactors.extend(split_result)
                    if len(split_interactors) != len(original_interactors):
                        print(f"[SPLIT] Bidirectional split: {len(original_interactors)} -> {len(split_interactors)} interactors in {json_key}", file=sys.stderr)
                    validated_payload[json_key]["interactors"] = split_interactors
        ...
        if PATHWAY_ASSIGNER_AVAILABLE and PathwayAssigner is not None and api_key:
            ...
            try:
                ...
                # NOTE: V2 Pathway Pipeline moved to AFTER database sync (STAGE 10.75)
                # This ensures interactions exist in DB before pathway assignment queries them

            except ImportError as e:
                print(f"[WARN] Failed to import pathway scripts: {e}", file=sys.stderr)
            except Exception as e:
                print(f"[ERROR] Pathway assignment failed: {e}", file=sys.stderr)
...
# --- STAGE 7.5: Validate arrows, directions, effects + extract direct mediator links ---
        ...
        # --- STAGE 11: Run V2 Pathway Pipeline (AFTER DB sync so interactions exist) ---
        if PATHWAY_ASSIGNER_AVAILABLE and flask_app is not None:
            current_step += 1
            update_status(
            ...
            try:
                ...
                print(f"\n{'='*80}", file=sys.stderr)
                print("RUNNING V2 PATHWAY PIPELINE", file=sys.stderr)
                print(f"{'='*80}", file=sys.stderr)

                # Step 1: Ensure Roots
                print(">> Step 1: Enforcing Roots...", file=sys.stderr)
                init_roots()

                # Step 2: Assign Initial Terms (guaranteed 100% coverage with retry cascade)
                print(">> Step 2: Assigning Initial Terms...", file=sys.stderr)
                assign_initial_terms()

                # Step 3: Refine (with recovery for any missing step2 assignments)
                print(">> Step 3: Refining Pathways...", file=sys.stderr)
                refine_pathways()

                # Step 4: Hierarchy (with recovery for any missing step3 assignments)
                print(">> Step 4: Building Hierarchy Backwards...", file=sys.stderr)
                build_hierarchy()

                # Step 5: Siblings
                print(">> Step 5: Discovering Siblings...", file=sys.stderr)
                discover_siblings()

                # Step 6: Reorganize (dedup, tree enforcement, hierarchy repair)
                print(">> Step 6: Reorganizing Pathways...", file=sys.stderr)
                reorganize_pathways()

                # Step 7: Verification (auto-fix minor issues, report status)
                print(">> Step 7: Verifying Pipeline...", file=sys.stderr)
                verification_result = verify()
                if verification_result.get('passed'):
                    print(f">> Verification PASSED ({verification_result.get('issues_fixed', 0)} auto-fixes applied)", file=sys.stderr)
                else:
                    print(f">> Verification FAILED - {verification_result.get('blocking_issues', 0)} blocking issues", file=sys.stderr)

                print(f"{'='*80}\n", file=sys.stderr)

            except ImportError as e:
                print(f"[WARN] Failed to import V2 pipeline scripts: {e}", file=sys.stderr)
            except Exception as e:
                print(f"[ERROR] V2 Pipeline failed: {e}", file=sys.stderr)
                import traceback
                traceback.print_exc(file=sys.stderr)
...
def run_requery_job(
    ...
    try:
        ...
        # --- STAGE 2.11: Split bidirectional interactors for requery ---
        if BIDIRECTIONAL_SPLIT_AVAILABLE and split_bidirectional_interactor is not None:
            main_protein_name = validated_new_payload.get("ctx_json", {}).get("main", user_query)
            for json_key in ["ctx_json", "snapshot_json"]:
                if json_key in validated_new_payload and "interactors" in validated_new_payload[json_key]:
                    original_interactors = validated_new_payload[json_key]["interactors"]
                    split_interactors = []
                    for interactor in original_interactors:
                        split_result = split_bidirectional_interactor(interactor, main_protein_name)
                        split_interactors.extend(split_result)
                    validated_new_payload[json_key]["interactors"] = split_interactors

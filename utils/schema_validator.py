...
def _reaggregate_interactor(interactor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for fn in functions:
        ...
        d = str(d).lower()
        if 'bid' in d:
            # Split bidirectional into both directions
    ...
    # Direction - stricter bidirectional check
...
def validate_schema_consistency(
    ...
    def _normalize_fn_direction(val: Any) -> str:
        s = str(val or "").strip().lower()
        if not s:
            return ""  # Preserve empty - don't assume main_to_primary
        if "bid" in s:
        ...
        return ""  # Unknown direction - leave empty for arrow determination to fill
...
def finalize_interaction_metadata(
    ...
    for interactor in interactors:
        ...
        if add_arrow_notation:
            ...
            # For split bidirectional interactors, add a tag to help frontend differentiate
